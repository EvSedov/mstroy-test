import { platformDetector, NetworkStatus } from "../services/PlatformDetector";
...
export class PlatformDetectorExample {
  ...
  demonstrateCapabilities(): void {
    ...
    console.log(
      "Cordova File поддержка:",
      capabilities.hasCordovaFile ? "✅" : "❌"
    );
    console.log(
      "Network Info поддержка:",
      capabilities.hasNetworkInfo ? "✅" : "❌"
    );
  ...
  demonstrateNetworkMonitoring(): void {
    ...
    this.networkUnsubscribe = platformDetector.instance.onNetworkChange(
    ...
    console.log(
      "Мониторинг сети активирован. Попробуйте отключить/включить интернет."
    );
  }
  ...
  demonstrateHelperMethods(): void {
    ...
    console.log(
      "File API:",
      platformDetector.instance.hasFileAPISupport()
        ? "Поддерживается"
        : "Не поддерживается"
    );
    console.log(
      "localStorage:",
      platformDetector.instance.hasLocalStorageSupport()
        ? "Доступен"
        : "Недоступен"
    );
    console.log(
      "Cordova File:",
      platformDetector.instance.hasCordovaFileSupport()
        ? "Доступен"
        : "Недоступен"
    );
    console.log(
      "Мобильное устройство:",
      platformDetector.instance.isMobileDevice() ? "Да" : "Нет"
    );
    console.log(
      "Touch поддержка:",
      platformDetector.instance.hasTouchSupport() ? "Есть" : "Нет"
    );
  }
  ...
  runFullDemo(): void {
    ...
    console.log(
      "💡 Для остановки мониторинга сети вызовите stopNetworkMonitoring()"
    );
  }
}
